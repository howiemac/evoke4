...
class DataObject(object):
  ...
  def update(self, args):
    ...
    x = [setattr(self, k, v) for k,v in args.items() if k in self._v_fields]
...
#      print ">>>>>>>>>>>>>> flushing only ",only
      keys = [i for i in changed.keys() if i in only]
      d = {}
      for k in keys: 
        d[k] = changed[k]
      changed = d
...
#    ob.update(data)
    for k,v in data.items(): # as per update(data) but allow extra data fields
      setattr(ob, k, v) 
  ...
  def old_list(cls, asObjects=True, sql='', like={}, isin={}, orderby='', where='' , limit='', pager=-1, pagelength=20, what='*',_debug=False,**criteria):
    ...
    if not sql:
      pairs = ob.quoted_pairs(criteria.items()).replace(',',' and ')
      likes = ob.quoted_pairs(like.items(), op='like').replace(',', ' and ')
      ...
      ins = ' and '.join('`%s` in %s' % (k,sql_list(v)) for k,v in isin.items())
 ...
 #         print k,v
      ...
      # like
      if like:
        like_sql, like_args = cls.sql_quoted_pairs(like.items(), op='like', link='and')
        like_sql=like_sql.replace("%","%%").replace('%%s','%s') # double the % wildcard (but not any %s)
        sqlparts.append(like_sql)
        sqlargs += like_args

      # isin
      for isin_field, isin_values in isin.items():
        if not isin_values: 
          continue
...
#    print "parse_orderby not implemented"
 ...
 # export / import
  ...
  def for_export(self,extras=[]):
    ...
    return dict((k,v) for k,v in self.__dict__.items() if k in self._v_fields or k in extras)
...
def makeDataClass(Schema):
    ...
    data = { #'db':db
      ...
      , '_v_fields': Schema._v_schema.keys()
