    ...
    if not hasattr(self,'_stage_data'):
      ...
      if len(data)==2:
        if lexists(self.file_loc()):
          data.append('%sx%s' % pim.open(open(self.file_loc())).size)
        else:
          data.append('?x?')
        save=True
      elif data[2][0]=='x':
        if lexists(self.file_loc()):
          data[2]='%sx%s' % pim.open(open(self.file_loc())).size
        else:
	  data[2]='?x?'
        save=True
      if len(data)==3:
        ...
        if lexists(self.thumb_loc()):
          data[3]='%sx%s' % pim.open(open(self.thumb_loc())).size
        else:
          data[3]='?x?'
        save=True
#      if len(data)==4:
